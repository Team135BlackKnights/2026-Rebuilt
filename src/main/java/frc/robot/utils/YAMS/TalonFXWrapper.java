...
//    m_dcmotorSim.ifPresent(dcMotorSim -> dcMotorSim.setAngle(angle.in(Radians)));
  ...
  m_pid = config.getPID();
    ...
    // Ensure PID/velocity-only closed loop also allocates and starts the notifier
    if (m_closedLoopControllerThread == null)
    {
      m_closedLoopControllerThread = new Notifier(this::iterateClosedLoopController);
      config.getTelemetryName().ifPresent(m_closedLoopControllerThread::setName);
    }
    if (config.getMotorControllerMode() == ControlMode.CLOSED_LOOP)
    {
      startClosedLoopController();
  ...
  public void setVoltage(Voltage voltage)
  {
    ...
    m_talonfx.setControl(m_softwareControlledReq.withOutput(m_simCommandVoltage));
  }

  private boolean usesSoftwareClosedLoopVoltageControl()
  {
    //currently always on? perhaps try returning false?
