...
import frc.robot.utils.YAMS.mechanisms.SmartVelocityMechanism;
...
public class FlyWheel extends SmartVelocityMechanism
{
  ...
  {
    return run(velocity).withName(" SetSpeed Supplier");
  }
  ...
  public Command setSpeed(AngularVelocity velocity)
  {
    ...
    return run(velocity).withName(" SetSpeed");
  }
  ...
  {
    return setSpeed(() -> velocity).withName(" SetSpeed");
  }
  ...
  {
    return Commands.runOnce(m_smc::startClosedLoopController)
                   .andThen(Commands.runOnce(() -> m_smc.setVelocity(velocity.get())))
                   .andThen(Commands.waitUntil(isNear(velocity.get(), tolerance).debounce(0.1, DebounceType.kRising)))
                   .withName(" RunToVelocity Supplier");
  }
  ...
  {
    return setSpeed(m_config.getAngularVelocity(speed)).withName(" RunSpeed");
  }
  ...
  public Trigger max()
  {
    ...
    throw new IllegalArgumentException("FlyWheel upper soft limit is empty");
  }
  ...
  public Trigger min()
  {
    ...
    throw new IllegalArgumentException("FlyWheel lower soft limit is empty");
  }
/* 
  @Override
  public Command sysId(Voltage maximumVoltage, Velocity<VoltageUnit> step, Time duration)
  {
    ...
    {
      throw new IllegalArgumentException("FlyWheel upper hard and motor controller soft limit is empty");
    }
    if (m_config.getLowerSoftLimit().isPresent())
    {
    ...
    {
      throw new IllegalArgumentException("FlyWheel lower hard and motor controller soft limit is empty");
    }
