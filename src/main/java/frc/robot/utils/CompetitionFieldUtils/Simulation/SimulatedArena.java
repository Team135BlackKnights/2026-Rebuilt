...
import frc.robot.utils.CompetitionFieldUtils.FieldObjects.GamePiece;
import frc.robot.utils.CompetitionFieldUtils.FieldObjects.GamePieceOnFieldSimulation;
import frc.robot.utils.CompetitionFieldUtils.FieldObjects.GamePieceProjectile;
import frc.robot.utils.CompetitionFieldUtils.Simulation.Rebuilt.Arena2026Rebuilt;
 ...
 * <p>
 * Check <a href=
 * 'https://shenzhen-robotics-alliance.github.io/maple-sim/using-the-simulated-arena/'>Online
 * Documentation</a>
 *
 * <h2>The heart of the simulator.</h2>
 *
 * <p>
 * This class cannot be instantiated directly; it must be created as a specific
 * season's arena.
 *
 * <p>
 ...
 *
 * <h2>The following objects can be added to the simulation world and will
 * interact with each other:</h2>
 *
 * <ul>
 * <li>{@link AbstractDriveTrainSimulation}: Represents abstract drivetrain
 * simulations with collision detection.
 * <li>{@link GamePieceOnFieldSimulation}: Represents abstract game pieces with
 * collision detection.
 * <li>{@link IntakeSimulation}: Represents an intake simulation that responds
 * to contact with
...
public abstract class SimulatedArena {
    /**
     * Whether to allow the simulation to run a real robot This feature is HIGHLY
    ...
    public NetworkTable redTable = NetworkTableInstance.getDefault()
            .getTable("SmartDashboard/MapleSim/MatchData/Breakdown/Red Alliance");
    public NetworkTable blueTable = NetworkTableInstance.getDefault()
            .getTable("SmartDashboard/MapleSim/MatchData/Breakdown/blue Alliance");
    public NetworkTable genericInfoTable = NetworkTableInstance.getDefault()
            .getTable("SmartDashboard/MapleSim/MatchData/Breakdown");

    public DoublePublisher matchClockPublisher = genericInfoTable.getDoubleTopic("Match Clock").publish();
     ...
     * @throws IllegalStateException if the method is call when running on a real
     *                               robot
     */
    public static SimulatedArena createArena(AbstractDriveTrainSimulation mainRobot) {
        ...
        if (instance == null)
            instance = new Arena2026Rebuilt(mainRobot);
     ...
     *
     * <p>
     * This method allows simulating an arena from a different year or a custom
     * field.
     *
     * <p>
     * Currently, only the 2024 arena is supported, so avoid calling this method for
     * now.
     *
     * @param newInstance the new simulation arena instance to override the current
     *                    one
     */
    public static void overrideInstance(SimulatedArena newInstance) {
        if (instance != null)
            instance = new Arena2026Rebuilt(newInstance.getMainDriveSimulation());
     ...
     *
     * <h4>If Using <a href=
     * 'https://github.com/Mechanical-Advantage/AdvantageKit'>Advantage-Kit</a>: DO
     * NOT CHANGE THE
     * DEFAULT TIMINGS</h4>
     *
     * <p>
     ...
     *
     * <p>
     * The new configuration will take effect the next time
     * {@link SimulatedArena#simulationPeriodic()} is called on
     * an instance.
     *
     * <p>
     * It is recommended to call this method before the first call to
     * {@link SimulatedArena#simulationPeriodic()} of
     * any instance.
     *
     * <p>
     * It is also recommended to keep the simulation frequency above 200 Hz for
     * accurate simulation results.
     *
     * @param robotPeriod                 the time between two calls of
     *                                    {@link #simulationPeriodic()}, usually
     *                                    obtained from
     *                                    {@link TimedRobot#getPeriod()}
     * @param simulationSubTicksPerPeriod the number of Iterations, or
     *                                    {@link #simulationSubTick(int)} that the
     *                                    simulation runs per each call to
    ...
    protected final AbstractDriveTrainSimulation mainDriveSimulation;
     ...
     *
     *
     * <h2>Constructs a new simulation arena with the specified field map of
     * obstacles.</h2>
     *
     * <p>
     * This constructor initializes a physics world with zero gravity and adds the
     * provided obstacles to the world.
     *
     * <p>
     * It also sets up the collections for drivetrain simulations, game pieces,
     * projectiles, and intake simulations.
     *
     * @param obstaclesMap the season-specific field map containing the layout of
     *                     obstacles for the simulation
     */
    protected SimulatedArena(FieldMap obstaclesMap, AbstractDriveTrainSimulation mainDriveSimulation) {
        ...
        this.mainDriveSimulation = mainDriveSimulation;
        this.driveTrainSimulations.add(mainDriveSimulation);
    ...
    public AbstractDriveTrainSimulation getMainDriveSimulation() {
        return mainDriveSimulation;
    }

    /**
     *
     *
     * <h2>Represents a custom simulation to be updated during each simulation
     * sub-tick.</h2>
     *
     * <p>
     * This allows you to register custom actions that will be executed at a high
     * frequency during each simulation
     * sub-tick. This is useful for tasks that need to be updated multiple times per
     * simulation cycle.
     *
     * <p>
     ...
     * <li>Adding custom simulation objects or handling events in the simulated
     * arena.
     * </ul>
     */
     ...
     *
     * <p>
     * <strong>NOTE:</strong> This method is automatically called in the constructor
     * of {@link IntakeSimulation}, so
     * you don't need to call it manually.
     *
     * <p>
     * The intake simulation should be bound to an
     * {@link AbstractDriveTrainSimulation} and becomes part of its
     * collision space.
     *
     * <p>
     * This method immediately starts the
     * {@link org.ironmaple.simulation.IntakeSimulation.GamePieceContactListener},
     ...
     *
     * <p>
     * The collision space of the drive train is immediately added to the simulation
     * world.
     *
     * <p>
     * Starting from the next call to {@link #simulationPeriodic()}, the
     * {@link AbstractDriveTrainSimulation#simulationSubTick()} method will be
     * called during each sub-tick of the
     * simulator.
     *
     ...
     *
     * <p>
     * The collision space of the game piece is immediately added to the simulation
     * world.
     *
     * <p>
     * {@link IntakeSimulation}s will be able to interact with this game piece
     ...
     *
     *
     * <h2>Tells the arena to start publishing the match breakdown data to network
     * tables</h2>
     */
     ...
     *
     *
     * <h2>Tells the arena to stop publishing the match breakdown data to network
     * tables</h2>
     */
     ...
     *
     *
     * <h2>replaces or adds a value to the match scoring breakdown published to
     * network tables</h2>
     *
     * @param isBlueTeam Wether to add to the blue teams match breakdown or the red
     *                   teams match breakdown
    ...
    public void replaceValueInMatchBreakDown(boolean isBlueTeam, String valueKey, Double value) {
        if (isBlueTeam)
            blueScoringBreakdown.put(valueKey, value);
        else
            redScoringBreakdown.put(valueKey, value);
    }

    /**
     *
     *
     * <h2>Defaults a value to 0 and creates it in the match breakdown. This is
     * useful on startup to make sure all match
     ...
     *
     *
     * <h2>Adds too a value in the scoring breakdown. If value does not already
     * exist in the scoring breakdown it will
    ...
    public void addValueToMatchBreakdown(boolean isBlueTeam, String ValueKey, Double toAdd) {
        if (isBlueTeam) {
            if (blueScoringBreakdown.get(ValueKey) == null)
                blueScoringBreakdown.put(ValueKey, toAdd);
            else
                blueScoringBreakdown.put(ValueKey, blueScoringBreakdown.get(ValueKey) + toAdd);
        } else {
            if (redScoringBreakdown.get(ValueKey) == null)
                redScoringBreakdown.put(ValueKey, toAdd);
            else
                redScoringBreakdown.put(ValueKey, redScoringBreakdown.get(ValueKey) + toAdd);
        }
    }
     ...
     *
     *
     * <h2>Registers a {@link GamePieceProjectile} to the Simulation and Launches
     * It.</h2>
     *
     * <p>
     * Calls to {@link GamePieceProjectile#launch()}, which will launch the game
     * piece immediately.
     *
     * @param gamePieceProjectile the projectile to be registered and launched in
     *                            the simulation
     */
     ...
     *
     * <p>
     * Removes the game piece from the physics world and the simulation's game piece
     * collection.
     *
     ...
     *
     * @param gamePieceLaunched the game piece projectile to be removed from the
     *                          simulation
     * @return <code>true</code> if this set contained the specified element
     */
     ...
     *
     *
     * <h2>Removes All {@link GamePieceOnFieldSimulation} Objects from the
     * Simulation.</h2>
     *
     * <p>
     * This method clears all game pieces from the physics world and the
     * simulation's game piece collection.
     */
     ...
     *
     *
     * <h2>Shuts down the current SimulatedArena and stops all simulation so that
     * simulatable objects may be added to a
     * new arena</h2>
     */
     ...
     *
     * <p>
     * This method should be called ONCE in {@link TimedRobot#simulationPeriodic()}
     * (or <code>
     ...
     * {@link SimulatedArena#overrideSimulationTimings(Time, int)} , the simulator
     * will
     * iterate through 5 Sub-ticks by default.
     *
     * <p>
     * The amount of CPU Time that the Dyn4j engine uses in displayed in <code>
     * SmartDashboard/MapleArenaSimulation/Dyn4jEngineCPUTimeMS</code>, usually
     ...
     *
     * <p>
     * This method performs the actions for each sub-tick of the simulation,
     * including:
     *
     * <ul>
     ...
     * <li>Removing any game pieces as detected by the {@link IntakeSimulation}
     * objects.
     ...
     *
     * @return all projectile pieces on the field as a set of GamePieceProjectile
     *         objects
     */
     ...
     *
     * <p>
     * Also, if you have a game-piece detection vision system
     * <strong>(wow!)</strong>, this is the how you can
     * simulate it.
     *
     * <p>
     * Both {@link GamePieceOnFieldSimulation} and {@link GamePieceProjectile} of
     * the specified type will be
     * included.
     *
     * <ul>
     * <li>The type is determined in the constructor of
     * {@link GamePieceOnFieldSimulation}.
     * <li>For example,
     * {@link org.ironmaple.simulation.seasonspecific.crescendo2024.CrescendoNoteOnField}
     * has the
     * type "Note".
     * </ul>
     *
     * @param type the type of game piece, as determined by the constructor of
     *             {@link GamePieceOnFieldSimulation}
     * @return a {@link List} of {@link Pose3d} objects representing the 3D
    ...
    public Pose2d getClosestRobotPose(Translation2d robotPosition) {
        AbstractDriveTrainSimulation closestRobot = null;
        double closestDistance = Double.MAX_VALUE;
        for (AbstractDriveTrainSimulation robot : driveTrainSimulations) {
    ...
    public Pose2d getClosestGamePiece(Class<?> wantedType,
            Translation2d robotPosition) {
        GamePieceOnFieldSimulation closestGamePiece = null;
        double closestDistance = Double.MAX_VALUE;
     ...
     *
     * <p>
     * This method clears all current game pieces from the field and places new game
     * pieces in their starting
     ...
     *
     * <p>
     * This method sets up the game pieces on the field, typically in their starting
     * positions for autonomous mode.
     *
     * <p>
     * It should be implemented differently for each season-specific subclass of
     * {@link SimulatedArena} to reflect
     ...
     *
     * <p>
     * For each season-specific subclass of {@link SimulatedArena}, there should be
     * a corresponding subclass of this
